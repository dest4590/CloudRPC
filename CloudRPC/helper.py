...
date_format = f'[green bold][{datetime.now().strftime("%d.%m.%Y")}][/] [violet bold]({datetime.now().strftime("%H:%M:%S")})[/]'
...
class Cats:
    ...
    def get_random(self):
        return [random.choice(self.randomWords), random.choice(self.random_gifs)]
